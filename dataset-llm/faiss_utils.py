...
def metadata_name(model_name: str, column_name: str, db_name: str) -> str:
    return f"faiss_metadata_{clean_model_name(model_name)}_{column_name}_{normalized_db_name(db_name)[:-len('.sqlite')]}.bin"


def models_and_params():
    return zip(["sentence-transformers/all-MiniLM-L6-v2", "nomic-ai/nomic-embed-text-v2-moe"], [384, 768], [{}, {"trust_remote_code": True, "device": "cpu"}])
