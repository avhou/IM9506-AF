...
def stage_2_tiktok(tiktok_db: str, min_duration: int = 30, max_duration: int = 60):
    ...
    conn.execute(f"""drop table if exists tiktok.videos_2nd_stage_optie_a;""")
    print(f"create table optie a")
    conn.execute(f"""create table if not exists tiktok.videos_2nd_stage_optie_a as select * from tiktok.videos where regexp_matches(video_description, '(?i)(russia|ukraine|russie|rusland|oekraine|oekraïne)') and video_duration >= {min_duration} and video_duration <= {max_duration};""")
    conn.execute(f"""alter table tiktok.videos_2nd_stage_optie_a add column transcription text;""")
...
if __name__ == "__main__":
    ...
    stage_2_tiktok(sys.argv[1], int(sys.argv[2]) if len(sys.argv) > 2 else 30, int(sys.argv[3]) if len(sys.argv) > 3 else 60)
