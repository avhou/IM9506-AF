...
def generate_index(hits: str, remove_index_first: bool = False):
    print(f"generating faiss index for input file {hits}")
    ...
    with sqlite3.connect(hits) as conn:
        result = conn.execute("select rowid, content, languages, url from hits order by rowid asc;").fetchall()
        rowids = [r[0] for r in result]
        documents = [r[1] for r in result]
        urls = [r[3] for r in result]
...
if __name__ == "__main__":
    ...
    generate_index(sys.argv[1], True)
